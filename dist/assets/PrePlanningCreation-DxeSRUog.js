...
import { a as p, B as K } from './axios-CgIVMX8t.js';
import './dayjs.min-DO4_1a-m.js';
import { R as M } from './ReuseableDataGrid-CdgPKZaX.js';
import { d as Z, a as ee } from './AddCircle-B9Pz-WR_.js';
import { T as ne, a as te, b as L, c as T } from './TabPanel-CBwhICZn.js';
import { T as g } from './TextField-BujPyVEk.js';
import { M as h } from './Skeleton-DFEmEML1.js';
const he = () => {
    ...
    d = a.useCallback(async () => {
      try {
        if (t.plannedDesignedId) {
          const s = (
            await p.get(
              `http://100.42.177.77:81/api/PrePlanning/GetPlanningHeaderListByDesignId?designId=${t.plannedDesignedId}`
            )
    ...
    a.useEffect(() => {
      (async () => {
        if (t.collectionId)
          try {
            const s = await p.get(
              `http://100.42.177.77:81/api/DesignRegistration/GetDesignListByCollectionId?CollectionId=${t.collectionId}`
            );
            F(s.data.result);
          } catch (s) {
    ...
    a.useEffect(() => {
      (async () => {
        if (t.plannedCollectionId)
          try {
            const s = await p.get(
              `http://100.42.177.77:81/api/PrePlanning/GetDesignFromPlanningHeaderByCollectionId?collectionid=${t.plannedCollectionId}`
            );
            W(s.data.result);
          } catch (s) {
