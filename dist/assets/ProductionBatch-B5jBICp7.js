...
import { I as M, B as ne, a as de } from './axios-BAxkGnGK.js';
import { d as le } from './AddCircleOutlineOutlined-rt-o6Sk6.js';
import {
  R as z,
  a as ce,
  b as ie,
...
import { d as fe } from './VisibilityOutlined-7xGDXOA6.js';
import { d as xe } from './Close-j8pmpgIQ.js';
import { T as u } from './TextField-CsNxJIZ0.js';
import { T as ge, a as Ie, b as W, c as q } from './TabPanel-CcXhfPsy.js';
import { M as be } from './Skeleton-C1EQF4m0.js';
var T = {},
  Ne = Z;
...
const Se = () => {
    ...
    r.useEffect(() => {
      m({
        ...
        orderNumber: (t == null ? void 0 : t.orderNumber) || '',
        launchDate:
          new Date(t == null ? void 0 : t.launchDate).toLocaleDateString(
            'en-GB',
    ...
    const [w, k] = r.useState([]),
      [P, O] = r.useState([]),
      { data: I, refetch: h } = F(),
      { data: b, refetch: l } = ee(i.productionId, { skip: !i.productionId }),
      [f, H] = r.useState([]);
    r.useEffect(() => {
      I && k(I.result.map((o, c) => ({ id: c + 1, ...o })));
    }, [I, h]),
      r.useEffect(() => {
        b && O(b.result.map((o, c) => ({ id: c + 1, ...o })));
      }, [b, l]),
      console.log('initialRows', w),
      console.log('batchDetailsRows', P);
    const x = (o) => {
      const { name: c, value: v } = o.target;
      if (c === 'collectionId') {
        const g = f.find((E) => E.collectionId === parseInt(v));
        m({ ...i, collectionId: v, orderNumber: g ? g.orderNumber : '' });
      } else m({ ...i, [c]: v });
    };
    console.log('formData', i);
    const [R, D] = r.useState(!1),
      B = (o) => {
        a(o), D(!0);
      },
      N = () => {
      ...
      S = [
        ...
        { field: 'remarks', headerName: 'Remarks' },
        { field: 'status', headerName: 'statusName' },
    ...
    return e.jsxs(G, {
      ...
      children: [
                ...
                children: [
                  ...
                  e.jsxs(ce, {
                    ...
                    children: [
                      ...
                      e.jsxs(ue, {
                        children: [
                          e.jsx(me, { id: 'alert-dialog-slide-description' }),
                          e.jsxs(n, {
                            container: !0,
                            spacing: 1,
                            width: 'Inherit',
        ...
        e.jsx(A, { color: '#cc8587', sx: { height: 1, width: '100%', mt: 2 } })
      ]
    });
  },
  Ee = () => {
    ...
    const [H, x] = r.useState('1'),
      R = (s, d) => {
        x(d);
      };
    r.useEffect(() => {
      f({
        productionId: (a == null ? void 0 : a.productionId) || 0,
        collectionId: (a == null ? void 0 : a.collectionId) || '',
        orderNumber: (a == null ? void 0 : a.orderNumber) || '',
        launchDate: (a == null ? void 0 : a.launchDate) || '',
        remarks: (a == null ? void 0 : a.remarks) || '',
    ...
    r.useState(!1);
    const { data: N, refetch: L } = re(),
      { data: S, refetch: o } = se(l.collectionId, { skip: !l.collectionId }),
      [c, v] = r.useState([]);
    r.useEffect(() => {
      N && v(N.result.map((s, d) => ({ id: d + 1, ...s })));
    }, [N, L]),
      r.useEffect(() => {
        S && B(S.result.map((s, d) => ({ id: d + 1, ...s })));
      }, [S, o]),
      console.log('initialRows', D);
    const g = (s) => {
        const { name: d, value: j } = s.target;
        if (d === 'collectionId') {
          const p = c.find((y) => y.collectionId === parseInt(j));
          f({ ...l, collectionId: j, orderNumber: p ? p.orderNumber : '' });
        } else f({ ...l, [d]: j });
      },
      E = () => {
        const s = {};
        return l.launchDate || (s.launchDate = 'Launch Date is required'), s;
      },
      X = async () => {
        const s = E();
    ...
    const Y = [
      ...
      J = r.useCallback(
        (s) => {
          ...
          const d = s
            .map((p) => {
              const y = h.current.getRow(p);
              return console.log('rowData', y), y ? y.planningHeaderId : null;
            })
            .filter((p) => p !== null);
          console.log('planningHeaderIds', d);
          const j = d.map((p) => ({
            prodctionDetId: 0,
            productionId: 0,
            planningHeaderId: p,
    ...
    return (
      r.useEffect(() => {
        f({ ...l, productionBatchDetails: m });
      }, [m]),
      r.useEffect(() => {
        h.current && console.log('API ref is ready:', h.current);
      }, [h]),
            ...
            children: [
                    ...
                    e.jsx(W, {
                      ...
                      sx: (s) => ({
                        '& .MuiTouchRipple-child': {
                          backgroundColor: `${s.palette.primary.main} !important`
                        }
                      })
                    }),
                    e.jsx(W, {
              ...
              e.jsxs(q, {
                ...
                children: [
                  ...
                  e.jsx(n, { container: !0, spacing: 2, width: 'Inherit' })
                ]
              }),
              e.jsx(q, { value: '2', children: e.jsx(Se, {}) })
            ]
          })
        })
      })
