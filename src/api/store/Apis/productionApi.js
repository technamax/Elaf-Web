...
export const productionApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    baseUrl: 'http://100.42.177.77:86/api/'
  }),
  endpoints: (builder) => ({
    ...
    getPendingReceivingByIssuanceId: builder.query({
      query: (issuanceId) =>
        `Receiving/GetPendingReceivingByIssuanceId?issuanceId=${issuanceId}`
    }),
    ...
    getInspectionHeader: builder.query({
      query: (poId) => `Receiving/GetInspectionHeaderByPoId?poId=${poId}`
    }),
...
export const {
  ...
  useGetPendingReceivingByIssuanceIdQuery,
