...
export default function PlanningProcess() {
    ...
    const GetLookUpDomains = async () => {
      try {
        const response = await axios.get(
          `http://100.42.177.77:8091/api/Common/GetLookUpDomains?appId=${1}`
        );
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.get(
        `http://100.42.177.77:8091/api/Common/SaveLookUp?lookupDomain=${formData.lookUpDomain}&LookUpName=${formData.lookUpName}&appId=1`
      );
